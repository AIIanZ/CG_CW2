...
static const int numVAOs = 2;
static const int numVBOs = 6;
...
GLuint textureIds[2] = { 0 }; // 新增数组存储5个纹理ID
...
//ImportedModel myModel("resources/CW.obj");
ImportedModel myModels[2] = {
	ImportedModel("resources/Blower.obj"),
	ImportedModel("resources/ControlboxAndCable.obj")
	//ImportedModel("resources/furnance.obj"),
	//ImportedModel("resources/Pump.obj"),
	//ImportedModel("resources/Trolley.obj")
	// 以此类推，加载所有五个模型
};
...
//		vector<glm::vec3> vert = myModels[modelIndex].getVertices();
//		vector<glm::vec2> text = myModels[modelIndex].getTextureCoords();
//		vector<glm::vec3> norm = myModels[modelIndex].getNormals();
...
//
//		for (int i = 0; i < myModels[modelIndex].getNumVertices(); i++)
//		{
...
void setupVertices(void) {
	...
	for (int modelIndex = 0; modelIndex < 2; modelIndex++) {
		...
		vector<float> pValues;  // 顶点位置值
		vector<float> tValues;  // 纹理坐标值
		vector<float> nValues;  // 法线值
		...
		// 绑定和填充顶点位置数据的VBO
		glBindBuffer(GL_ARRAY_BUFFER, vbo[modelIndex * 3]);
		glBufferData(GL_ARRAY_BUFFER, pValues.size() * sizeof(float), &pValues[0], GL_STATIC_DRAW);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(0);

		// 绑定和填充纹理坐标数据的VBO
		glBindBuffer(GL_ARRAY_BUFFER, vbo[modelIndex * 3 + 1]);
		glBufferData(GL_ARRAY_BUFFER, tValues.size() * sizeof(float), &tValues[0], GL_STATIC_DRAW);
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(1);

		// 绑定和填充法线数据的VBO
		glBindBuffer(GL_ARRAY_BUFFER, vbo[modelIndex * 3 + 2]);
		glBufferData(GL_ARRAY_BUFFER, nValues.size() * sizeof(float), &nValues[0], GL_STATIC_DRAW);
...
void init(GLFWwindow* window)
{
	...
	setupVertices();
	//shuttleTextureId = Utils::loadTexture("resources/pic.jpeg");

		textureIds[0] = Utils::loadTexture("resources/RootsBlower.png");
		textureIds[1] = Utils::loadTexture("resources/controlBoxTexture.png");
		//textureIds[2] = Utils::loadTexture("resources/Furnace.png");
		//textureIds[3] = Utils::loadTexture("resources/DiffusionPump.png");
		//textureIds[4] = Utils::loadTexture("resources/trolley.png");
		// 加载每个模型的纹理
...
//	mMat = glm::rotate(mMat, toRadians(0.f), glm::vec3(0.f, 0.f, 1.f));
//
//
//	//这句必须要有，否则鼠标中建失效
...
void display(GLFWwindow* window, double currentTime) {
	...
	for (int modelIndex = 0; modelIndex < 2; modelIndex++) {
		...
		// 启用纹理
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureIds[modelIndex]);
		...
		// 绘制模型
		glDrawArrays(GL_TRIANGLES, 0, myModels[modelIndex].getNumVertices());
	}
